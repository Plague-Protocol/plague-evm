    ...
    <main className="min-h-screen" style={{ backgroundColor: '#0a0e27', color: '#f0f4f8' }}>
      ...
      >
        <div className="pointer-events-none absolute inset-0" aria-hidden="true">
          <div className="absolute left-1/4 top-1/2 h-80 w-80 -translate-y-1/2 rounded-full opacity-15 blur-3xl"
            style={{ backgroundColor: '#a855f7' }} />
          <div className="absolute right-1/4 top-1/2 h-64 w-64 -translate-y-1/2 rounded-full opacity-10 blur-3xl"
        ...
        <div className="relative mx-auto w-full max-w-6xl text-center">
          <span
            ...
            style={{ borderColor: 'rgba(168,85,247,0.4)', backgroundColor: 'rgba(168,85,247,0.08)', color: '#a855f7' }}
          >
            Rules & Instructions
          </span>
          <h1 className="mt-6 font-display text-6xl font-bold leading-none sm:text-7xl lg:text-8xl" style={{ color: '#f0f4f8' }}>
            HOW TO PLAY
          </h1>
          <p className="mx-auto mt-6 max-w-2xl font-mono text-base leading-relaxed" style={{ color: '#7a8592' }}>
      ...
      <div className="mx-auto w-full max-w-6xl space-y-20 px-6 py-16">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {roles.map((role) => (
              ...
              >
                ...
                <p className="mt-3 font-mono text-sm leading-relaxed" style={{ color: '#b0bcc9' }}>
                  {role.desc}
                </p>
                <div
                ...
                >
                  <p className="font-mono text-[10px] uppercase tracking-[0.2em]" style={{ color: '#7a8592' }}>Win Condition</p>
        ...
        <section>
          ...
          <p className="mt-3 font-mono text-sm leading-relaxed" style={{ color: '#7a8592' }}>
            ...
            {[
              ...
              <div key={item.status} className="flex gap-4">
                <div className="flex flex-col items-center">
                  ...
                  {i < arr.length - 1 && (
                    <div className="w-px flex-1" style={{ backgroundColor: 'rgba(168,85,247,0.2)', minHeight: '2rem' }} />
                  )}
                </div>
                <div className="pb-8">
                  <div className="flex items-center gap-3">
                  ...
                  <p className="mt-2 font-mono text-sm leading-relaxed" style={{ color: '#b0bcc9' }}>{item.desc}</p>
                  <ul className="mt-3 space-y-1">
                    {item.actions.map((a) => (
                      <li key={a} className="flex items-start gap-2 font-mono text-xs" style={{ color: '#7a8592' }}>
            ...
            {phases.map((phase) => (
              ...
              >
                ...
                <div className="mt-3 flex items-baseline gap-2">
                  <span className="font-mono text-[10px] uppercase tracking-[0.2em]" style={{ color: '#7a8592' }}>{phase.number}</span>
                  <h3 className="font-display text-xl font-bold" style={{ color: phase.color }}>{phase.name}</h3>
                </div>
                <p className="mt-2 font-mono text-xs leading-relaxed" style={{ color: '#9aabb8' }}>{phase.desc}</p>
              </div>
            ))}
          </div>
        </section>
          ...
          >
            <ul className="space-y-3">
              {patientZeroSuccession.map((rule, i) => (
                <li key={i} className="flex items-start gap-3 font-mono text-sm" style={{ color: '#b0bcc9' }}>
                  <span
            ...
            >
              <p className="font-mono text-xs uppercase tracking-[0.15em]" style={{ color: '#a855f7' }}>On-Chain Verifiable</p>
              <p className="mt-1 font-mono text-xs" style={{ color: '#9aabb8' }}>
                The current Patient Zero address is stored publicly in the contract (<code className="font-mono" style={{ color: '#06b6d4' }}>currentPatientZero[roomId]</code>). Any player can verify who holds the role at any time.
              </p>
            </div>
          </div>
        </section>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-mono text-sm font-bold leading-snug" style={{ color: '#f0f4f8' }}>{rule.title}</h3>
                  </div>
                </div>
                <div
                ...
                <p className="mt-2 font-mono text-xs leading-relaxed" style={{ color: '#9aabb8' }}>{rule.desc}</p>
              </div>
            ))}
          </div>
          <div
        ...
        <section>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {proofRules.map((rule) => (
              ...
              >
                ...
                <h3 className="mt-3 font-mono text-sm font-bold" style={{ color: '#06b6d4' }}>{rule.title}</h3>
                <p className="mt-2 font-mono text-xs leading-relaxed" style={{ color: '#9aabb8' }}>{rule.desc}</p>
              </div>
            ))}
          </div>
          <div
            ...
            style={{ borderColor: 'rgba(168,85,247,0.3)', backgroundColor: 'rgba(168,85,247,0.07)' }}
          >
            <p className="font-mono text-xs font-bold uppercase tracking-[0.15em]" style={{ color: '#a855f7' }}>How the ZK Circuit Works</p>
            <p className="mt-2 font-mono text-xs leading-relaxed" style={{ color: '#9aabb8' }}>
              ...
              Your commitment <code style={{ color: '#06b6d4' }}>Poseidon(role, secret)</code> was registered on-chain at game start.
              The nullifier <code style={{ color: '#06b6d4' }}>Poseidon(secret, roomId, round)</code> ensures the same proof cannot be replayed across rounds.
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {endgame.map((e) => (
              ...
              >
                ...
                >
                  <p className="font-mono text-[10px] uppercase tracking-[0.18em]" style={{ color: '#7a8592' }}>Condition</p>
                  <p className="mt-1 font-mono text-xs" style={{ color: '#f0f4f8' }}>{e.condition}</p>
                </div>
                <div
                  className="mt-2 rounded border px-3 py-2"
                  style={{ borderColor: 'rgba(132,204,22,0.25)', backgroundColor: 'rgba(132,204,22,0.07)' }}
                >
                  <p className="font-mono text-[10px] uppercase tracking-[0.18em]" style={{ color: '#7a8592' }}>Payout</p>
              ...
              >
                ...
                <ul className="mt-4 space-y-2">
                  {section.tips.map((tip, i) => (
                    <li key={i} className="flex items-start gap-2 font-mono text-xs" style={{ color: '#9aabb8' }}>
        ...
        <section className="text-center">
          <div
            ...
            style={{ borderColor: 'rgba(168,85,247,0.3)', backgroundColor: 'rgba(168,85,247,0.07)' }}
          >
            <h2 className="font-display text-4xl font-bold" style={{ color: '#f0f4f8' }}>Ready to Play?</h2>
            <p className="mt-3 font-mono text-sm" style={{ color: '#7a8592' }}>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
              <Link
                ...
                style={{ background: 'linear-gradient(135deg, #a855f7, #06b6d4)', color: '#f0f4f8' }}
              >
                Browse Lobby
              </Link>
              <Link
...
function SectionTitle({ number, title }: { number: string; title: string }) {
  return (
    <div className="flex items-baseline gap-4">
      <span className="font-mono text-[10px] uppercase tracking-[0.28em]" style={{ color: '#a855f7' }}>{number}</span>
      <h2 className="font-display text-3xl font-bold sm:text-4xl" style={{ color: '#f0f4f8' }}>{title}</h2>
    </div>
  )
}
